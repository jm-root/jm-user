...
exports.default = function () {
  ...
  var o = {
    ...
    validate: function validate(opts) {
      return _bluebird2.default.resolve().then(function () {
        if (opts.account && !isNaN(opts.account)) throw _jmErr2.default.err(Err.FA_INVALID_ACCOUNT);
        if (opts.email && !_validator2.default.isEmail(opts.email)) throw _jmErr2.default.err(Err.FA_INVALID_EMAIL);
        if (opts.mobile && !isMobile(opts.mobile)) throw _jmErr2.default.err(Err.FA_INVALID_MOBILE);
        return null;
      });
    },
    ...
    updateUser: function updateUser(id, opts, cb) {
      ...
      return this.validate(opts).then(function () {
    ...
    signon: function signon(username, password, cb) {
      ...
      return this.findUser(username).then(function (doc) {
        if (!doc) throw _jmErr2.default.err(Err.FA_USER_NOT_EXIST);
        if (!doc.active) throw _jmErr2.default.err(Err.FA_ACCOUNT_BAN);
    ...
    signup: function signup(opts, cb) {
      var self = this;
      if (cb) {
        this.signup(opts).then(function (doc) {
          cb(null, doc);
        }).catch(function (err) {
          cb(err);
        });
      ...
      // 允许游客注册
      if (!query.length) {
        return self.user.create(data).then(function (doc) {
      ...
      return this.validate(data).then(function () {
        ...
        if (doc) return _bluebird2.default.reject(_jmErr2.default.err(Err.FA_USER_EXIST));
  ...
  var cb = function cb(db) {
    ...
    o.user = (0, _user2.default)(o, opts);
    o.avatar = (0, _avatar2.default)(o, opts);
    o.ready = true;
    o.emit('ready');
  };
