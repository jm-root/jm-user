...
exports.default = function () {
  ...
  var signon = function signon(opts, cb) {
    service.signon(opts.data.username, opts.data.password).then(function (doc) {
      cb(null, doc);
    }).catch(function (err) {
      var doc = Err.FAIL;
  ...
  var signup = function signup(opts, cb) {
    if (opts.ip) {
  ...
  this.onReady().then(function () {
    router.add('/signon', 'post', signon).add('/signup', 'post', signup).use('/users/:id/avatar', (0, _avatar2.default)(service, opts)).add('/users/:id/exists', 'get', function (opts, cb) {
      service.findUser(opts.params.id, function (err, doc) {
        if (!doc) return cb(null, { ret: 0 });
        cb(null, { ret: doc.id });
      });
    }).add('/users', 'post', signup).add('/users/:id', 'post', function (opts, cb) {
      ...
      service.updatePassword(opts.params.id, opts.data.oldPassword, opts.data.password).then(function (doc) {
        cb(null, {
          ret: 1
        });
      }).catch(function (err) {
      ...
      next();
    }).use('/users', (0, _jmMsDaorouter2.default)(service.user, {
      list: listOpts,
      get: getOpts
    }));
  });
